...
export default function Home() {
	...
	const organizersInView = useInView(organizersRef, { amount: 0.15, once: true })
	const joinInView = useInView(joinRef, { amount: 0.3, once: true })
	...
	return (
		<Main>
			<Hero>
				...
				>
					DEV<HeroWordmarkAccent>x</HeroWordmarkAccent>
				</HeroWordmark>
				<HeroTagline
					as={motion.p}
					initial={{ opacity: 0, y: 16 }}
				...
				<PillarsGrid>
					{pillars.map((pillar, index) => (
						<PillarCard
							...
							animate={{ opacity: pillarsInView ? 1 : 0, y: pillarsInView ? 0 : 24 }}
			...
			<AboutSection ref={aboutRef}>
				<AboutGrid>
					<AboutText
						...
						animate={{ opacity: aboutInView ? 1 : 0, x: aboutInView ? 0 : -24 }}
			...
			<ContentSection ref={organizersRef}>
				...
				<OrganizerGrid>
					{organizers.map((organizer, index) => (
						<OrganizerCard
							...
							initial={{ opacity: 0, y: 24 }}
							animate={{
								opacity: organizersInView ? 1 : 0,
								y: organizersInView ? 0 : 24
							}}
			...
			<JoinSection ref={joinRef}>
				<JoinPanel
					as={motion.div}
					initial={{ opacity: 0, y: 24 }}
					animate={{ opacity: joinInView ? 1 : 0, y: joinInView ? 0 : 24 }}
					transition={{ duration: 0.6, ease: "easeOut" }}
				>
					...
					<JoinText>
						Free to attend, open to every skill level. Come meet the San Diego developer community.
					</JoinText>
					<Button href={nextEventLink} variant="primary" size="default">
						Join the Next Event
					</Button>
...
const SectionTitle = styled.h2<{ $center?: boolean; $onDark?: boolean }>`
	font-size: clamp(1.75rem, 4vw, 2.75rem);
	font-weight: 800;
	margin: 0 0 1.5rem 0;
	color: ${(props) => (props.$onDark ? "var(--color-accent-ink)" : "var(--color-ink)")};
	text-align: ${(props) => (props.$center ? "center" : "left")};
	max-width: 720px;
